...
AgendaService::~AgendaService() {
//	delete storage_;	lead to bug when call ~Storage() function
}
...
bool AgendaService::userRegister(std::string userName, std::string password,
	...
	if (tmp.size() > 0) {
		...
		storage_->sync();	// for multiple process share file user.csv source
		return true;
	}
}
...
void AgendaService::quitAgenda(void) {
	std::cout << "Call AgendaService quitAgenda!" << std::endl;
	if (storage_ != NULL) {
		if (!storage_->sync()) {
