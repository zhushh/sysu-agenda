...
#else
    read_password(userPassword_, "[password] : ");
#endif
...
void AgendaUI::changeStartTime() {
    ...
    if (agendaSocket_.recvSTR() == "OK") {
        agendaSocket_.sendSTR(newStartTime);
    }
    if (agendaSocket_.recvSTR() == "OK") {
        agendaSocket_.sendSTR("OK");
    }
...
void AgendaUI::changeEndTime() {
    ...
    if (agendaSocket_.recvSTR() == "OK") {
        agendaSocket_.sendSTR(newEndTime);
    }
    if (agendaSocket_.recvSTR() == "OK") {
        agendaSocket_.sendSTR("OK");
    }
...
void AgendaUI::changeUserPassword(void) {
    string oldPasswd, newPasswd_confirm, newPasswd;
    cout << "[change user password]" << endl;

#ifndef PASSWORD
    oldPasswd = readString("[old password]: ");
    newPasswd = readString("[new password]: ");
    newPasswd_confirm = readString("[confirm new password]: ");
#else
    read_password(oldPasswd, "[old password]: ");
    read_password(newPasswd, "[new password]: ");
    read_password(newPasswd_confirm, "[confirm new password]: ");
#endif
    ...
    if (oldPasswd != userPassword_) {
    ...
    } else if (newPasswd_confirm != newPasswd) {
        agendaSocket_.sendSTR("q");
        if (agendaSocket_.recvSTR() == "OK")
        ...
        agendaSocket_.sendSTR(userName_);
        if (agendaSocket_.recvSTR() == "OK") {
            agendaSocket_.sendSTR(oldPasswd);
        }
        if (agendaSocket_.recvSTR() == "OK") {
            agendaSocket_.sendSTR(newPasswd);
        }
        if (agendaSocket_.recvSTR() == "OK") {
            agendaSocket_.sendSTR("OK");
        }
        if (agendaSocket_.recvSTR() == "true") {
            userPassword_ = newPasswd;
